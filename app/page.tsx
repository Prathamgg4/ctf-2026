    ...
    <main className="min-h-screen bg-[#050014] text-[#a586ff] font-mono flex flex-col items-center relative overflow-x-hidden overflow-y-auto">
      ...
      <div className="z-20 flex flex-col items-center w-full max-w-7xl px-4 sm:px-8 lg:px-12 pt-6 lg:pt-10 flex-grow">
        <div className="flex flex-col lg:flex-row items-stretch gap-8 lg:gap-12 w-full mb-16 lg:mb-24">
          ...
          <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} className="flex-1 w-full">
            <div className="bg-[#0b0e14]/90 backdrop-blur-3xl border border-[#a586ff]/30 rounded-[1.5rem] lg:rounded-[2.5rem] p-6 lg:p-12 relative shadow-[0_0_100px_rgba(0,0,0,0.8)] h-full">
              <div className="flex justify-between items-start mb-10 lg:mb-14">
                <div>
                  <h1 className="text-4xl sm:text-5xl lg:text-7xl font-black italic tracking-tighter text-white leading-none uppercase">CTF<span className="text-[#a586ff]">.26</span></h1>
                  <p className="text-[8px] lg:text-[10px] tracking-[0.4em] text-[#a586ff] mt-4 font-bold uppercase">{">"} UPLINK STATUS: SECURE</p>
                </div>
                <div className="p-3 lg:p-4 bg-[#a586ff]/5 rounded-xl lg:rounded-2xl border border-[#a586ff]/20">
                  <Activity className="text-[#a586ff] animate-pulse" size={24} />
                </div>
              </div>

              <div className="grid grid-cols-4 gap-2 lg:gap-4 mb-8 lg:mb-12">
                {Object.entries(timeLeft).map(([unit, val]) => (
                  <div key={unit} className="bg-black/40 border border-[#a586ff]/10 p-3 lg:p-5 rounded-xl lg:rounded-2xl text-center">
          ...
          {/* RULES BOX (SYNC-GLOW RESTORED) */}
          <motion.div 
            initial={{ opacity: 0, x: 40 }} 
            animate={{ 
              opacity: 1, x: 0, 
              borderColor: isTyping ? "rgba(165, 134, 255, 0.7)" : "rgba(165, 134, 255, 0.2)", 
              boxShadow: isTyping ? "0 0 50px rgba(165, 134, 255, 0.2)" : "none" 
            }} 
            className="w-full lg:w-[450px] bg-[#0b0e14]/90 backdrop-blur-3xl border rounded-[1.5rem] lg:rounded-[2.5rem] p-8 lg:p-12 flex flex-col transition-all duration-500"
          >
            <h2 className="text-2xl lg:text-3xl font-black text-white mb-8 flex items-center gap-4 italic uppercase tracking-tighter">
              <ShieldAlert className="text-[#a586ff]" size={28} /> Rules
            </h2>
            <div className="space-y-6 lg:space-y-10">
              <div className="flex gap-4 lg:gap-6 group">
                <span className="flex-shrink-0 w-8 h-8 lg:w-10 lg:h-10 rounded-lg lg:rounded-xl bg-[#a586ff]/5 border border-[#a586ff]/20 flex items-center justify-center text-[#a586ff] font-black group-hover:bg-[#a586ff] group-hover:text-black transition-all">1</span>
                <p className="text-[#a586ff]/60 text-[10px] lg:text-xs leading-relaxed font-bold uppercase tracking-wider">Challenges are worth dynamic points based on difficulty.</p>
              </div>
              <div className="flex gap-4 lg:gap-6 group">
                <span className="flex-shrink-0 w-8 h-8 lg:w-10 lg:h-10 rounded-lg lg:rounded-xl bg-[#a586ff]/5 border border-[#a586ff]/20 flex items-center justify-center text-[#a586ff] font-black group-hover:bg-[#a586ff] group-hover:text-black transition-all">2</span>
                <p className="text-[#a586ff]/60 text-[10px] lg:text-xs leading-relaxed font-bold uppercase italic tracking-wider">Infrastructure tampering results in instant perma-ban.</p>
              </div>
            </div>
            <div className="mt-auto pt-6 border-t border-[#a586ff]/10 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[7px] lg:text-[8px] font-black text-[#a586ff]/40 uppercase tracking-widest">{isTyping ? 'Syncing...' : 'Standby'}</span>
              </div>
      ...
      <footer className="z-10 w-full relative">
        <img src="/footer-brand.jpeg" alt="HACKX" className="w-full h-auto block drop-shadow-[0_0_20px_rgba(165,134,255,0.3)]" />
      </footer>

      {/* TERMS MODAL (ALL SECTIONS RESTORED) */}
      <AnimatePresence>
        {showTerms && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/95 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#0b0e14] border border-[#a586ff]/40 w-full max-w-2xl rounded-[1.5rem] lg:rounded-[2.5rem] overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <div className="p-6 lg:p-8 border-b border-[#a586ff]/10 flex items-center justify-between bg-[#a586ff]/5">
                <div className="flex items-center gap-4 text-[#a586ff]">
                  <FileText size={20} />
                  <h3 className="text-lg lg:text-xl font-black italic uppercase tracking-tighter">Code_of_Conduct_v1.0</h3>
                </div>
              ...
              <div ref={scrollRef} onScroll={handleScroll} className="p-6 lg:p-10 overflow-y-auto space-y-8 text-[#a586ff]/70 text-[11px] lg:text-sm font-bold leading-relaxed custom-scrollbar">
                ...
                <section className="space-y-4">
                  <h4 className="text-white tracking-[0.3em] text-xs uppercase underline">D. Penalties</h4>
